...
impl<'a> App<'a> {
    pub fn new(window: config::GraphicsConfig) -> App<'a> {
        ...
        let glyph_cache = GlyphCache::new("./resources/fonts/PxPlus_IBM_VGA8.ttf", (), TextureSettings::new())
    ...
    pub fn update(&mut self, args: UpdateArgs) {
        ...
        if self.state.powerup_cooldown > 0.0 {
            self.state.powerup_cooldown -= args.dt;
        }
